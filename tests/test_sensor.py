...
class TestSensorPlatformSetup:
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_creates_back_wash_frequency_sensor_when_reported(
        self, pure_mock_hass, pure_mock_config_entry, pure_mock_coordinator
    ):
        """Back-wash frequency sensor is created once the robot has reported it.

        backWashTime/backWashType are no longer separate sensors — see
        select.robot_self_clean_interval and its own tests instead.
        """
        pure_mock_hass.data[DOMAIN] = {
            pure_mock_config_entry.entry_id: pure_mock_coordinator
        }
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_skips_back_wash_frequency_sensor_when_never_reported(
        self, pure_mock_hass, pure_mock_config_entry, pure_mock_coordinator
    ):
        """A dock without a wash cycle never sends this field — skip it."""
        pure_mock_hass.data[DOMAIN] = {
            pure_mock_config_entry.entry_id: pure_mock_coordinator
        }
        mock_add_entities = MagicMock()
